...
export interface ComparisonResult {
  ...
  rows: {
    ...
    item2Value: string;
    item3Value?: string;
    winnerIndex?: number; // 0, 1, or 2 (optional)
  }[];
  verdict: string;
  bestFor: string[]; // ["Travel", "Budget", "Durability"] corresponding to items
}
...
export interface MeetingDetails {
  isAgreed: boolean;
  location: string;
  dateTime: string; // ISO string or loose description
  summary: string;
}

// Fixed processVoiceCommand to follow @google/genai guidelines
export const processVoiceCommand = async (
  base64Audio: string,
  mimeType: string
): Promise<VoiceSearchIntent | null> => {
  try {
    // Using the native audio model for best speech understanding
    ...
    const textPrompt = `
      ...
      Categories available: [${Object.values(Category).join(', ')}]
    ...
    return JSON.parse(text) as VoiceSearchIntent;
...
// Fixed generateListingMetadata to follow @google/genai guidelines
export const generateListingMetadata = async (
  ...
  try {
    ...
    return JSON.parse(text) as ListingMetadata;
...
// Fixed identifyItemFromImage to follow @google/genai guidelines
...
// Upgraded checkProductSafety to Gemini 3 Pro for advanced safety reasoning and recall database lookup
export const checkProductSafety = async (
  ...
  try {
    ...
    const result = JSON.parse(text) as SafetyCheckResult;

    // Extract Grounding Sources (URLs) as per Search Grounding guidelines
    ...
    return {
      isSafe: true, // Non-blocking fall back
...
// Fixed analyzeDeal to include source extraction for search grounding compliance
export const analyzeDeal = async (
  ...
  try {
    ...
    const text = response.text;
    if (!text) return null;
    const result = JSON.parse(text) as DealAnalysis;

    // Extract grounding chunks for search grounding compliance
    const sources: { title: string; uri: string }[] = [];
    if (response.candidates?.[0]?.groundingMetadata?.groundingChunks) {
      response.candidates[0].groundingMetadata.groundingChunks.forEach((chunk: any) => {
        if (chunk.web?.uri && chunk.web?.title) {
...
// Upgraded compareListings to Gemini 3 Pro for advanced comparative reasoning
...
// Fixed askConcierge to follow @google/genai guidelines for chat and parts
export const askConcierge = async (
  ...
  try {
    ...
    // Summarize inventory to fit in context efficiently
    const inventory = listings
      .filter(l => !l.isSold)
      .map(l => `ID: ${l.id} | ${l.title} | $${l.price} | ${l.category} | ${l.condition}`)
      .join('\n');
    ...
    // Construct history for Gemini using the correct parts structure
    ...
    return JSON.parse(text) as ConciergeResponse;
...
// Fixed generateSmartReplies to follow @google/genai guidelines
export const generateSmartReplies = async (
  ...
  try {
    ...
    const historyText = fullHistory.slice(-5).map(m => m.text).join('\n');
...
// Fixed optimizeListingDescription to follow @google/genai guidelines
export const optimizeListingDescription = async (
  ...
  try {
    ...
    console.error("Description optimizer failed", e);
    return null;
  }
};

// Fixed generateInspectionChecklist to follow @google/genai guidelines
...
// Fixed summarizeUserReputation to follow @google/genai guidelines
export const summarizeUserReputation = async (
  ...
  try {
    const model = "gemini-3-flash-preview";
    const reviewTexts = reviews.map(r => `"${r.comment}" (${r.rating}/5)`).join("\n");
    ...
    console.error("Reputation summary failed", e);
    return null;
  }
};

// Fixed extractMeetingDetails to follow @google/genai guidelines
export const extractMeetingDetails = async (
  ...
  try {
    const model = "gemini-3-flash-preview";
    const history = messages.slice(-10).map(m => m.text).join("\n");
    ...
    const text = response.text;
    if(!text) return null;
    return JSON.parse(text) as MeetingDetails;
  } catch (e) {
    console.error("Meeting extraction failed", e);
    return null;
  }
};
...
const getDefaultChecklist = (category: string): string[] => {
  switch(category) {
    case Category.STROLLERS: return ["Check brakes", "Test folding mechanism", "Inspect harness", "Check wheels"];
    case Category.CAR_SEATS: return ["Check expiration date", "Inspect for stress marks", "Verify harness tightens", "Check buckle function"];
    case Category.TOYS: return ["Check for loose parts", "Inspect battery compartment", "Check for sharp edges", "Verify sound/lights"];
    default: return ["Item matches description", "No undisclosed damage", "Clean condition", "All parts included"];
  }
};
